...
public class PedidoCompraService {
    ...
    @Transactional(readOnly=true) public String nomeArquivo(Long cotacaoId,Long pedidoId,String extensao){PedidoCompra pedido=buscarPedidoAtual(cotacaoId,pedidoId);String nome=java.util.stream.Stream.of(limparNomeArquivo(pedido.getNomeDistribuidora()),limparNomeArquivo(pedido.getCotacao().getNome())).filter(v->!v.isBlank()).collect(Collectors.joining(" - "));if(nome.isBlank())nome=pedido.getNumero();return nome.substring(0,Math.min(nome.length(),180))+"."+extensao;}
    ...
    private String limparNomeArquivo(String valor){return valor==null?"":valor.replaceAll("[\\\\/:*?\"<>|\\p{Cntrl}]","-").replaceAll("\\s+"," ").trim().replaceAll("[. ]+$","");}
}
